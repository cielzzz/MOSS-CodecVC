#!/usr/bin/env bash
# Batch-47 fixed no_text quick20 orchestration on the local RTX4090 host.
#
...
#
#   ACTION=once ALLOW_CODECVC_BATCH47_LOCAL_EVAL=1 sh scripts/ver3_1/run_batch47_local_quick20.sh
#
# To wait safely and process step 500..3000 as markers arrive:
#
#   ACTION=watch ALLOW_CODECVC_BATCH47_LOCAL_EVAL=1 sh scripts/ver3_1/run_batch47_local_quick20.sh
#
...
RUNNER="$ROOT/scripts/ver3_1/run_batch47_local_quick20.py"

TASK_NAME="codecVC-ver3-1-batch47-local-quick20-20260716"
CHECKPOINT_ROOT="$ROOT/outputs/ver3_1_batch47_ddlfm_no_text_probe_20260716"
OUTPUT_ROOT="$ROOT/testset/outputs/codecVC-ver3-1-batch47-local-quick20-20260716"
RECORD_ROOT="$ROOT/trainset/local_jobs/codecVC-ver3-1-batch47-local-quick20-20260716"

ACTION="${ACTION:-plan}"
POLL_SECONDS="${POLL_SECONDS:-60}"
MAX_SCANS="${MAX_SCANS:-0}"
ALLOW_RUN="${ALLOW_CODECVC_BATCH47_LOCAL_EVAL:-0}"
...
if [ "$ACTION" != "plan" ]; then
  [ "$ALLOW_RUN" = "1" ] || {
    echo "ERROR: local GPU evaluation is guarded; set ALLOW_CODECVC_BATCH47_LOCAL_EVAL=1" >&2
    exit 2
  }
  args+=(--allow-run)
fi
