#!/usr/bin/env bash
# Discover and safely schedule the paired Batch-43 full320 evaluations.
#
# The registered schedule contains only step-20000 and step-30000.  A later
...
# the exact same step.  004101 then performs all scientific and QZ audits.
#
# Safe one-shot status/plan:
#   MODE=once ACTION=plan bash scripts/004102_watch_batch43_paired_full320.sh
#
# Platform dry-run when the next paired checkpoint is ready:
#   MODE=monitor ACTION=dry-run bash scripts/004102_watch_batch43_paired_full320.sh
#
# Live operation is double-gated and submits at most one evaluation at once:
#   MODE=monitor ACTION=submit ALLOW_LIVE_SUBMIT=1 \
#     bash scripts/004102_watch_batch43_paired_full320.sh
...
STATE_ROOT="${STATE_ROOT:-$PROJECT_ROOT/trainset/qz_jobs/ver23_batch43_paired_full320_scheduler_${STAMP}}"
EVAL_ROOT="${EVAL_ROOT:-$PROJECT_ROOT/testset/outputs/ver23_batch43_paired_full320_${STAMP}}"
SUBMIT_WRAPPER="${SUBMIT_WRAPPER:-$PROJECT_ROOT/scripts/004101_submit_batch43_paired_full320_qz.sh}"
...
if [ "$ACTION" != "plan" ]; then
  [ -s "$SUBMIT_WRAPPER" ] || die "missing Batch-43 full320 submit wrapper: $SUBMIT_WRAPPER"
...
if errors:
    raise SystemExit("Batch-43 full320 watcher training audit failed:\n- " + "\n- ".join(errors))
print(f"[batch43-full320-watch] training provenance r3={r3_job} r5={r5_job}")
PY
}

record_root_for() {
  printf '%s/trainset/qz_jobs/ver23_batch43_paired_full320_step%s_%s\n' "$PROJECT_ROOT" "$1" "$STAMP"
}
...
seen = set()
for step in steps:
    record = project_root / f"trainset/qz_jobs/ver23_batch43_paired_full320_step{step}_{stamp}"
    ...
    if payload.get("schema") != "batch43_paired_full320_v1" or payload.get("status") != "complete" or int(payload.get("step", -1)) != step:
...
lines = [
    "# Batch-43 paired full320 schedule",
    "",
...
print(f"[batch43-full320-watch] rollup completed={completed}/{len(steps)} metrics_rows={len(metrics)}")
PY
}
...
scan_once() {
  refresh_rollup
  local step record completion submitted active_step="" next_step="" probe status detail_r3 detail_r5
  ...
  for step in $STEPS; do
    ...
    completion="$record/COMPLETED.json"
    if [ -s "$completion" ] && [ -s "$(step_root_for "$step")/aggregate/paired_metrics.tsv" ]; then
  ...
  if [ -n "$active_step" ]; then
    echo "[batch43-full320-watch] active_step=$active_step waiting for completion; no new submission"
    return 0
  fi
  if [ -z "$next_step" ]; then
    ...
    if [ "$completed_count" = "2" ]; then
      echo "[batch43-full320-watch] all registered full320 checkpoints are complete"
      return 10
    fi
    echo "[batch43-full320-watch] no paired checkpoint ready"
    return 0
  fi

  echo "[batch43-full320-watch] next_ready_step=$next_step"
  case "$ACTION" in
    plan)
      echo "[batch43-full320-watch] plan only; would run STEP=$next_step via $SUBMIT_WRAPPER"
      ;;
    dry-run)
      local dry_marker
      dry_marker="$(record_root_for "$next_step")/dry_run.ok"
      if [ -s "$dry_marker" ]; then
        echo "[batch43-full320-watch] step-$next_step already passed platform dry-run"
      else
      ...
      ;;
    submit)
      STEP="$next_step" DRY_RUN=0 CONFIRM_BATCH43_FULL320=1 \
        MIN_CHECKPOINT_AGE_SEC="$MIN_CHECKPOINT_AGE_SEC" bash "$SUBMIT_WRAPPER"
...
while true; do
  ...
  echo "[batch43-full320-watch] scan=$scan_count utc=$(date -u +%Y-%m-%dT%H:%M:%SZ) action=$ACTION"
  set +e
  scan_once
  rc=$?
  set -e
  if [ "$rc" = "10" ] && [ "$STOP_WHEN_COMPLETE" = "1" ]; then
    echo "[batch43-full320-watch] registered schedule complete; stopping monitor"
    break
  fi
  if [ "$rc" != "0" ] && [ "$rc" != "10" ]; then
    exit "$rc"
  ...
  if [ "$MAX_SCANS" -gt 0 ] && [ "$scan_count" -ge "$MAX_SCANS" ]; then
    echo "[batch43-full320-watch] MAX_SCANS=$MAX_SCANS reached"
    break
  fi
  sleep "$POLL_SECONDS"
done
