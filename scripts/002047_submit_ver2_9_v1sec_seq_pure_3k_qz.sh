#!/usr/bin/env bash
# Submit Batch-26: V1-second architecture + old data + Fix 6 speaker sequence.
...
export POST_TRAIN_EVAL_LABEL="${POST_TRAIN_EVAL_LABEL:-v1sec_seq_pure}"
export JOB_NAME_PREFIX="${JOB_NAME_PREFIX:-codecvc-ver2-9-v1sec-seq-pure-olddata}"
export OUT_DIR="${OUT_DIR:-$ROOT/outputs/lora_runs/ver2_9_v1sec_seq_pure_olddata_steps${MAX_TRAIN_STEPS}}"
export BATCH_ID="${BATCH_ID:-v1sec-seq-pure-olddata-$(date -u +%Y%m%d-%H%M%S)}"
export DRY_RUN

echo "[v1sec-seq-pure-submit] train_jsonl_spec=$TRAIN_JSONL_SPEC"
echo "[v1sec-seq-pure-submit] no_text_repeat=$NO_TEXT_REPEAT text_repeat=$TEXT_REPEAT max_train_steps=$MAX_TRAIN_STEPS"
echo "[v1sec-seq-pure-submit] prepared_dir=$PREPARED_DIR out_dir=$OUT_DIR"
