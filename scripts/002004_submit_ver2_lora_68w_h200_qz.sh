...
LORA_WARMUP_FREEZE_STEPS="${LORA_WARMUP_FREEZE_STEPS:-0}"
...
REF_AUDIO_CFG_DROPOUT="${REF_AUDIO_CFG_DROPOUT:-0.0}"
...
CONTENT_CROSS_ATTN_LR_MULTIPLIER="${CONTENT_CROSS_ATTN_LR_MULTIPLIER:-1.0}"
CONTENT_ENCODER_HIDDEN_SIZE="${CONTENT_ENCODER_HIDDEN_SIZE:-0}"
...
echo "[qz-train] lr_scheduler_type=$LR_SCHEDULER_TYPE warmup_ratio=$WARMUP_RATIO lora_warmup_freeze_steps=$LORA_WARMUP_FREEZE_STEPS"
...
echo "[qz-train] content_cross_attn=$ENABLE_CONTENT_CROSS_ATTN layers=$CONTENT_CROSS_ATTN_LAYERS feature_dim=$CONTENT_CROSS_ATTN_FEATURE_DIM gate_init=$CONTENT_CROSS_ATTN_GATE_INIT dropout=$CONTENT_CROSS_ATTN_DROPOUT output_scale=$CONTENT_CROSS_ATTN_OUTPUT_SCALE lr_multiplier=$CONTENT_CROSS_ATTN_LR_MULTIPLIER encoder_hidden_size=$CONTENT_ENCODER_HIDDEN_SIZE encoder_layers=$CONTENT_ENCODER_LAYERS guided_weight=$GUIDED_ATTN_LOSS_WEIGHT guided_warmup=$GUIDED_ATTN_WARMUP_STEPS guided_band=$GUIDED_ATTN_BAND_FRAMES phoneme_weight=$PHONEME_CLASSIFIER_LOSS_WEIGHT"
...
CONTENT_CROSS_ATTN_ARGS="\$CONTENT_CROSS_ATTN_ARGS --content-cross-attn-lr-multiplier $CONTENT_CROSS_ATTN_LR_MULTIPLIER"
CONTENT_CROSS_ATTN_ARGS="\$CONTENT_CROSS_ATTN_ARGS --content-encoder-hidden-size $CONTENT_ENCODER_HIDDEN_SIZE"
...
"\$PY" -m accelerate.commands.launch \\
  ...
  --ref-audio-cfg-dropout "$REF_AUDIO_CFG_DROPOUT" \\
  ...
  --lora-warmup-freeze-steps "$LORA_WARMUP_FREEZE_STEPS" \\
  \$RESUME_ARGS \\
...
echo "  CONTENT_CROSS_ATTN_LR_MULTIPLIER=$CONTENT_CROSS_ATTN_LR_MULTIPLIER"
echo "  CONTENT_ENCODER_HIDDEN_SIZE=$CONTENT_ENCODER_HIDDEN_SIZE"
...
echo "  REF_AUDIO_CFG_DROPOUT=$REF_AUDIO_CFG_DROPOUT"
...
echo "  LORA_WARMUP_FREEZE_STEPS=$LORA_WARMUP_FREEZE_STEPS"
