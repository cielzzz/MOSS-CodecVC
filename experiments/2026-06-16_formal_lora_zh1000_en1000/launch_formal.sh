...
TRAIN_JSONL="${TRAIN_JSONL:-${SCRIPT_DIR}/data/train_target_zh1000_en1000.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/outputs/lora_runs/formal_lora_zh1000_en1000_formal}"

if [ ! -f "${TRAIN_JSONL}" ]; then
  echo "missing target formal dataset: ${TRAIN_JSONL}" >&2
  echo "prepare a true zh1000 + en1000 file first" >&2
  exit 2
fi
